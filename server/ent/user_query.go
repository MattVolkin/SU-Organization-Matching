...
// QueryLedClubs chains the current query on the "led_clubs" edge.
func (_q *UserQuery) QueryLedClubs() *ClubQuery {
	query := (&ClubClient{config: _q.config}).Query()
...
func (_q *UserQuery) QueryAnswers() *AnswerQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLedClubs: _q.withLedClubs.Clone(),
...
// WithLedClubs tells the query-builder to eager-load the nodes that are connected to
// the "led_clubs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithLedClubs(opts ...func(*ClubQuery)) *UserQuery {
	query := (&ClubClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withLedClubs = query
	return _q
}
...
func (_q *UserQuery) WithAnswers(opts ...func(*AnswerQuery)) *UserQuery {
	query := (&AnswerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withLedClubs; query != nil {
		if err := _q.loadLedClubs(ctx, query, nodes,
			func(n *User) { n.Edges.LedClubs = []*Club{} },
			func(n *User, e *Club) { n.Edges.LedClubs = append(n.Edges.LedClubs, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadLedClubs(ctx context.Context, query *ClubQuery, nodes []*User, init func(*User), assign func(*User, *Club)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.LedClubsTable)
		s.Join(joinT).On(s.C(club.FieldID), joinT.C(user.LedClubsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.LedClubsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.LedClubsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "led_clubs" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *UserQuery) loadAnswers(ctx context.Context, query *AnswerQuery, nodes []*User, init func(*User), assign func(*User, *Answer)) error {
